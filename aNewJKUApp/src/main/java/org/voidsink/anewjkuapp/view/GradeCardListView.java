 ...
 */
public class GradeCardListView extends StickyListHeadersListView {

    protected static String TAG = GradeCardListView.class.getCanonicalName();
...
//    protected int list_card_layout_resourceID = R.layout.calendar_card_layout;
    ...
    public GradeCardListView(Context context) {
        super(context);
        init(null, 0);
    }

    public GradeCardListView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public GradeCardListView(Context context, AttributeSet attrs, int defStyle) {
...
//            list_card_layout_resourceID = a.getResourceId(R.styleable.card_options_list_card_layout_resourceID, this.list_card_layout_resourceID);
    ...
    public void setAdapter(StickyListHeadersAdapter adapter) {
        if (adapter instanceof GradeCardArrayAdapter) {
            setAdapter((GradeCardArrayAdapter) adapter);
        } else {
     ...
     */
    public void setAdapter(GradeCardArrayAdapter adapter) {
        super.setAdapter(adapter);

        //Set Layout used by items
//        adapter.setRowLayoutId(R.layout.card_layout);

        adapter.setGradeListView(this);
        mAdapter = adapter;
    }
}
