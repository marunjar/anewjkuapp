...
public class KusssHandler {
	...
	private static final String SELECT_MY_GRADES = "body.intra > table > tbody > tr > td > table > tbody > tr > td.contentcell > div.contentcell > table > tbody > tr:has(td)";
	...
	public List<ExamGrade> getGrades() {
		...
		try {
			...
			GradeType type = null;
			for (Element row : rows) {
				Elements headers = row.getElementsByTag("h3");
				if (headers.size() == 1) {
					type = GradeType.parseGradeType(headers.get(0).text());
				} else {
					ExamGrade grade = new ExamGrade(type, row);
